...
def abs_attn_core(q_head, k_head, v_head, attn_mask, dropatt, is_training,
    ...
    if attn_mask is not None:
        attn_score = attn_score - 10000.0 * attn_mask
...
def rel_attn_core(q_head, k_head_h, v_head_h, k_head_r, seg_embed, seg_mat,
    ...
    if seg_mat is None:
        ...
        # [L, batch, head, dim] x [2, head, dim] = [L, batch, head, 2]
        ef = tf.einsum('ibnd,snd->ibns', q_head + r_s_bias, seg_embed)
        # [L, M+L, batch, 2] x [L, batch, head, 2] = [L, M+L, batch, head]
...
def two_stream_rel_attn(h, g, r, mems, r_w_bias, r_r_bias, seg_mat, r_s_bias,
    ...
    with tf.variable_scope(scope, reuse=True):
        ...
        if target_mapping is not None:
            # [pred_size, batch, head, dim] x [pred_size, len, batch]
...
def transformer_xl(inp_k, n_token, n_layer, d_model, n_head,
                   ...
                   float16=False, scope='transformer'):
    """
    ...
    tf_float = tf.float16 if float16 else tf.float32
    tf.logging.info('Use float type {}'.format(tf_float))

    new_mems = []
    with tf.variable_scope(scope, custom_getter=get_custom_getter(tf.float16 if float16 else tf.float32),
                           reuse=tf.AUTO_REUSE):
...
def lm_loss(hidden, target, n_token, d_model, initializer, lookup_table=None,
            tie_weight=False, use_tpu=False):
    """doc."""
...
class RunConfig(object):
    ...
    def __init__(self, is_training, use_tpu, float16, dropout, dropatt,
...
class XLNetModel(object):
    ...
    def __init__(self, xlnet_config, run_config, input_ids, seg_ids, input_mask,
        ...
        with tf.variable_scope("model", reuse=tf.AUTO_REUSE):
            (self.output, self.new_mems, self.lookup_table) = transformer_xl(**tfm_args)
...
def get_qa_outputs(FLAGS, inputs, is_training):
    """Loss for downstream span-extraction QA tasks such as SQuAD."""

    inp = tf.transpose(inputs["input_ids"], [1, 0])
    seg_id = tf.transpose(inputs["segment_ids"], [1, 0])
    inp_mask = tf.transpose(inputs["input_mask"], [1, 0])
    ...
    with tf.variable_scope("start_logits",
                           custom_getter=get_custom_getter(tf.float16 if FLAGS.float16 else tf.float32),
                           reuse=tf.AUTO_REUSE):
        start_logits = tf.layers.dense(output, 1, kernel_initializer=initializer)
        start_logits = tf.transpose(tf.squeeze(start_logits, -1), [1, 0])
        start_logits_masked = start_logits * (1 - p_mask) - 10000.0 * p_mask
        start_log_probs = tf.nn.log_softmax(start_logits_masked, -1)

    # logit of the end position
    with tf.variable_scope("end_logits", custom_getter=get_custom_getter(tf.float16 if FLAGS.float16 else tf.float32),
                           reuse=tf.AUTO_REUSE):
        if is_training:
            ...
            start_index = tf.one_hot(start_positions, depth=seq_len, axis=-1,
                                     dtype=tf.float16 if FLAGS.float16 else tf.float32)
            start_features = tf.einsum("lbh,bl->bh", output, start_index)
            start_features = tf.tile(start_features[None], [seq_len, 1, 1])
            end_logits = tf.layers.dense(tf.concat([output, start_features], axis=-1), xlnet_config.d_model,
            ...
            end_logits = tf.layers.dense(end_logits, 1, kernel_initializer=initializer, name="dense_1")
            end_logits = tf.transpose(tf.squeeze(end_logits, -1), [1, 0])
            end_logits_masked = end_logits * (1 - p_mask) - 10000.0 * p_mask
            end_log_probs = tf.nn.log_softmax(end_logits_masked, -1)
        else:
            # during inference, compute the end logits based on beam search

            start_top_log_probs, start_top_index = tf.nn.top_k(start_log_probs, k=FLAGS.start_n_top)
            start_index = tf.one_hot(start_top_index, depth=seq_len, axis=-1,
                                     dtype=tf.float16 if FLAGS.float16 else tf.float32)
            ...
            end_logits_masked = end_logits * (1 - p_mask[:, None]) - 10000.0 * p_mask[:, None]
            end_log_probs = tf.nn.log_softmax(end_logits_masked, -1)
            end_top_log_probs, end_top_index = tf.nn.top_k(end_log_probs, k=FLAGS.end_n_top)
            end_top_log_probs = tf.reshape(end_top_log_probs, [-1, FLAGS.start_n_top * FLAGS.end_n_top])
