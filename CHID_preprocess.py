...
'''
import os, collections, pickle, random
import pandas as pd
import numpy as np
from tqdm import tqdm
import sentencepiece as spm
import json
from preprocess.prepro_utils import encode_pieces
# import ipdb
...
train_data_file = os.path.join(path, 'origin_data/CHID/train.json')
train_label_file = os.path.join(path, 'origin_data/CHID/train_answer.json')
dev_data_file = os.path.join(path, 'origin_data/CHID/dev.json')
dev_label_file = os.path.join(path, 'origin_data/CHID/dev_answer.json')
test_data_file = os.path.join(path, 'origin_data/CHID/test.json')
test_label_file = os.path.join(path, 'origin_data/CHID/test_answer.json')
out_data_file = os.path.join(path, 'origin_data/CHID/out.json')
out_label_file = os.path.join(path, 'origin_data/CHID/out_answer.json')
idiom_file = os.path.join(path, 'origin_data/CHID/idiomDict.json')
vocab_file = os.path.join(path, 'check_points/pretrain_models/bert_wwm_ext_base/vocab.txt')
...
idiom_dict = json.load(open(idiom_file))
...
def read_chid_examples(input_data_file, input_label_file, is_training=True):
    ...
    for data in tqdm(input_data):
        ...
        for context in data['content']:
            ...
            #     if context[index:index + 6] == "#idiom":
            #         is_blank = True
            #     if context[index - 12:index + 1].startswith("#idiom"):
            #         is_blank = False
...
def read_chid_xlnet_examples(input_data_file, input_label_file, is_training=True):
    '''
    将原始数据处理为如下形式：
    part_passage遍历每个blak的周围位置
    :param input_data:
    ...
    for data in tqdm(input_data):
        ...
        for context in data['content']:
            ...
            tags = re.findall(r'#idiom[0-9]{6}#', context)
            for tag in tags:
                context = context.replace(tag, "✡")
            sp_tokens = encode_pieces(sp_model, context)
            sp_tokens_ = []
            for token in sp_tokens:
                if '✡' in token and len(token) > 1:
                    ...
                    sp_tokens_.append(token)
            sp_tokens = sp_tokens_
            doc_tokens = []
            tag_i = 0
            for j in range(len(sp_tokens)):
                if sp_tokens[j] == '✡':
                    doc_tokens.append(tags[tag_i])
                    tag_i += 1
                else:
                    doc_tokens.append(sp_tokens[j])
...
def convert_examples_to_xlnet_features(examples, max_seq_length=64, max_num_choices=10):
    '''
    将所有候选答案放置在片段开头
    '''
    ...
    def _loop(example, unique_id, label):
        ...
        tag = example.tag
        all_doc_tokens = list(example.doc_tokens)

        pos = all_doc_tokens.index(tag)

        for i, elem in enumerate(example.options):
            option = encode_pieces(sp_model, elem)
            num_tokens = max_tokens_for_doc - (len(option) + 1)  # <eod>和segA的成语
            tmp_l, tmp_r = add_tokens_for_around(all_doc_tokens, pos, num_tokens)
            num_l = len(tmp_l)
            num_r = len(tmp_r)
            ...
            for token in tmp_r:
                if '#idiom' in token and token != tag:
                    tokens_r.append('<mask>')
                else:
                    tokens_r.append(token)
            tokens_r = tokens_r[: num_r]
            del tmp_r
            tokens = tokens_l + ['<eod>'] + tokens_r + ['<sep>'] + option + ['<sep>'] + ['<cls>']

            input_id = [_piece_to_id(token) for token in tokens]
            input_mask = [0] * len(input_id)
            segment_id = [0] * (len(tokens_l) + 1 + len(tokens_r) + 1) + [1] * (len(option) + 1) + [2]
            cls_ind = len(tokens) - 1
...
def convert_examples_to_idiom_features(examples, tokenizer, max_seq_length=128, max_num_choices=10):
    '''
    将所有候选答案放置在片段开头
    '''
    ...
    def _loop(example, unique_id, label):
        ...
        pos = all_doc_tokens.index(tag)
        for i, elem in enumerate(example.options):
            idiom_text = idiom_dict[elem][:idiom_length - 1]
            elem_ = elem + ':'
            elem_ += idiom_text
            option = tokenizer.tokenize(elem_)
            num_tokens = max_tokens_for_doc - len(option) - 1  # [unused1]和segA的成语和：
            tmp_l, tmp_r = add_tokens_for_around(all_doc_tokens, pos, num_tokens)
            num_l = len(tmp_l)
            num_r = len(tmp_r)
...
def generate_input(data_file, label_file, example_file, feature_file, tokenizer, max_seq_length, max_num_choices,
    ...
    if os.path.exists(feature_file):
        ...
        # features = convert_examples_to_idiom_features(examples, tokenizer, max_seq_length, max_num_choices)
...
def generate_xlnet_input(data_file, label_file, example_file, feature_file, max_seq_length, max_num_choices,
                         is_training=True):
    if os.path.exists(feature_file):
        features = pickle.load(open(feature_file, 'rb'))
    elif os.path.exists(example_file):
        examples = pickle.load(open(example_file, 'rb'))
        features = convert_examples_to_xlnet_features(examples, max_seq_length, max_num_choices)
        pickle.dump(features, open(feature_file, 'wb'))
    else:
        examples = read_chid_xlnet_examples(data_file, label_file, is_training=is_training)
        pickle.dump(examples, open(example_file, 'wb'))
        features = convert_examples_to_xlnet_features(examples, max_seq_length, max_num_choices)
        pickle.dump(features, open(feature_file, 'wb'))
...
if __name__ == '__main__':
    ...
    train_examples_file = f"dataset/CHID/train_examples_{max_seq_length}.pkl"
    train_features_file = f'dataset/CHID/train_features_{max_seq_length}.pkl'
    train_features = generate_input(train_data_file, train_label_file, train_examples_file, train_features_file,
                                    tokenizer, max_seq_length, max_num_choices, is_training=True)

    dev_examples_file = f"dataset/CHID/dev_examples_{max_seq_length}.pkl"
    dev_features_file = f'dataset/CHID/dev_features_{max_seq_length}.pkl'
    dev_features = generate_input(dev_data_file, dev_label_file, dev_examples_file, dev_features_file, tokenizer,
                                  max_seq_length,
                                  max_num_choices, is_training=False)

    test_examples_file = f"dataset/CHID/test_examples_{max_seq_length}.pkl"
    test_features_file = f'dataset/CHID/test_features_{max_seq_length}.pkl'
    test_features = generate_input(test_data_file, test_label_file, test_examples_file, test_features_file, tokenizer,
                                   max_seq_length, max_num_choices, is_training=False)

    out_examples_file = f"dataset/CHID/out_examples_{max_seq_length}.pkl"
    out_features_file = f'dataset/CHID/out_features_{max_seq_length}.pkl'
    out_features = generate_input(out_data_file, out_label_file, out_examples_file, out_features_file, tokenizer,
                                  max_seq_length, max_num_choices, is_training=False)
...
#     sp_model.Load('check_points/pretrain_models/xlnet_mid/spiece.model')
#
#     train_examples_file = f"dataset/CHID/xlnet_train_examples_{max_seq_length}.pkl"
#     train_features_file = f'dataset/CHID/xlnet_train_features_{max_seq_length}.pkl'
#     train_features = generate_xlnet_input(train_data_file, train_label_file, train_examples_file, train_features_file,
#                                           max_seq_length, max_num_choices, is_training=True)
#
#     dev_examples_file = f"dataset/CHID/xlnet_dev_examples_{max_seq_length}.pkl"
#     dev_features_file = f'dataset/CHID/xlnet_dev_features_{max_seq_length}.pkl'
#     dev_features = generate_xlnet_input(dev_data_file, dev_label_file, dev_examples_file, dev_features_file,
#                                         max_seq_length,
#                                         max_num_choices, is_training=False)
#
#     test_examples_file = f"dataset/CHID/xlnet_test_examples_{max_seq_length}.pkl"
#     test_features_file = f'dataset/CHID/xlnet_test_features_{max_seq_length}.pkl'
#     test_features = generate_xlnet_input(test_data_file, test_label_file, test_examples_file, test_features_file,
#                                          max_seq_length, max_num_choices, is_training=False)
#
#     out_examples_file = f"dataset/CHID/xlnet_out_examples_{max_seq_length}.pkl"
#     out_features_file = f'dataset/CHID/xlnet_out_features_{max_seq_length}.pkl'
#     out_features = generate_xlnet_input(out_data_file, out_label_file, out_examples_file, out_features_file,
#                                         max_seq_length, max_num_choices, is_training=False)
