...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    _PrelimPrediction = collections.namedtuple(  # pylint: disable=invalid-name
        "PrelimPrediction",
        ["feature_index", "start_index", "end_index", "start_logit", "end_logit", "target_logits"])
    ...
    for (example_index, example) in enumerate(tqdm(all_examples)):
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            if version_2_with_negative:
                ...
                if feature_null_score < score_null:
                    ...
                    null_target_logits = result.target_logits
        ...
        if version_2_with_negative:
            prelim_predictions.append(
                _PrelimPrediction(
                    ...
                    target_logits=null_target_logits))
        ...
        _NbestPrediction = collections.namedtuple(  # pylint: disable=invalid-name
            "NbestPrediction", ["text", "start_logit", "end_logit", "target_logits"])
        ...
        for pred in prelim_predictions:
            ...
            if pred.start_index > 0:  # this is a non-null prediction
                ...
                no_ans_ind = np.argmax(pred.target_logits)
                final_text = ""
                if no_ans_ind == 0:
                    final_text = ""  # UNKNOWN
                elif no_ans_ind == 1:
        ...
        if version_2_with_negative:
            ...
            if len(nbest) == 1:
                nbest.insert(0, _NbestPrediction(text="", start_logit=0.0, end_logit=0.0, target_logits=[0, 0, 0]))

        # In very rare edge cases we could have no valid predictions. So we
        # just create a nonce prediction in this case to avoid failure.
        if not nbest:
            nbest.append(_NbestPrediction(text="", start_logit=0.0, end_logit=0.0, target_logits=[0, 0, 0]))

        assert len(nbest) >= 1

        total_scores = []
        best_non_null_entry = None
        best_null_entry = None
        for entry in nbest:
        ...
        for (i, entry) in enumerate(nbest):
            ...
            output['target_logits'] = [float(entry.target_logits[0]),
                                       float(entry.target_logits[1]),
                                       float(entry.target_logits[2])]
